        ...
        If hit.RowIndex >= 0 AndAlso hit.Type = DataGridViewHitTestType.RowHeader Then
            Try
                ...
                If row.Cells("Estado").Value IsNot Nothing AndAlso Not IsDBNull(row.Cells("Estado").Value) Then
                    estadoTexto = row.Cells("Estado").Value.ToString()
                End If

                If estadoTexto = "Aplicada" OrElse estadoTexto = "Cancelada" OrElse estadoTexto = "Parcialmente aplicada" Then
                    MsgBox("Esta amonestación no puede modificarse porque ya tiene descuentos aplicados.", MsgBoxStyle.Information, "Registro bloqueado")
                    ResetFormFields()
                    Return
                End If
