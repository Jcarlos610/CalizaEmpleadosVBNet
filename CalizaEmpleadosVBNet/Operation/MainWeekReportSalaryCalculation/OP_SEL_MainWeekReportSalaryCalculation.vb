...
Public Class OP_SEL_MainWeekReportSalaryCalculation
    ...
    Private Function ValidarProdPlant() As Boolean
        ...
        If Integer.TryParse(valor, numero) AndAlso numero >= 80 AndAlso numero <= 100 Then
            Return True
        Else
            MessageBox.Show("Solo se permiten valores entre 80 y 100.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            TB_ProdPlant.Focus()
            TB_ProdPlant.SelectAll()
            Return False
        End If
    ...
    Private Sub LoadWeekInformation()
        ...
        Dim CLCheck As New CL_Payroll
        Dim ultimaAprobacion As DataTable = CLCheck.GetLatestApprovalByWeek(startDate, endDate)
        If ultimaAprobacion.Rows.Count > 0 AndAlso ultimaAprobacion.Rows(0)("Status").ToString() = "Rechazado" Then
            Dim motivo As String = ultimaAprobacion.Rows(0)("RejectionReason").ToString()
            MessageBox.Show("Esta semana fue RECHAZADA anteriormente." & vbCrLf & vbCrLf &
                             "Motivo: " & motivo & vbCrLf & vbCrLf &
    ...
    Private Sub BuildWeeklyGrid_Movements(sourceTable As DataTable, startDate As Date)
        ...
        EmployeesInfo.Columns.Add("Desc. Domingo", GetType(String))
        ...
        'EmployeesInfo.Columns.Add("B. Comida Final", GetType(String))
        EmployeesInfo.Columns.Add("Bono BP", GetType(String))
        EmployeesInfo.Columns.Add("Amonest..", GetType(Decimal))
        EmployeesInfo.Columns.Add("Dias Amonest. a Descontar", GetType(Decimal))
        EmployeesInfo.Columns.Add("Dias Excedente Amonest.", GetType(Decimal))
        EmployeesInfo.Columns.Add("Desc. Amonest. BP", GetType(String))
        EmployeesInfo.Columns.Add("Bono BP Final", GetType(String))
        ...
        EmployeesInfo.Columns.Add("Monto D. Transporte", GetType(String))
        EmployeesInfo.Columns.Add("D. Transporte", GetType(Decimal))
        EmployeesInfo.Columns.Add("Transporte", GetType(String))
        'EmployeesInfo.Columns.Add("Desc. Transporte", GetType(String))
        'EmployeesInfo.Columns.Add("Transporte Final", GetType(String))
        EmployeesInfo.Columns.Add("Transporte entre Empleados", GetType(String))
        'EmployeesInfo.Columns.Add("Transporte Fijo 1", GetType(String))
        'EmployeesInfo.Columns.Add("Transporte Fijo 2", GetType(String))
        'EmployeesInfo.Columns.Add("Desc. Transporte entre Empleados", GetType(String))
        'EmployeesInfo.Columns.Add("Transporte entre Empleados Final", GetType(String))
        EmployeesInfo.Columns.Add("Monto B. Botonero Temp", GetType(String))
        EmployeesInfo.Columns.Add("Desc. Botonero Temp", GetType(String))
        EmployeesInfo.Columns.Add("Botonero Temp Final", GetType(String))
        EmployeesInfo.Columns.Add("Monto B. Botonero Fijo", GetType(String))
        EmployeesInfo.Columns.Add("Desc. Botonero Fijo", GetType(String))
        ...
        EmployeesInfo.Columns.Add("No. Horas A.", GetType(String))
        EmployeesInfo.Columns.Add("Desc. Horas A.", GetType(String))
        ...
        DGV_CompleteWeekInfo.Columns("Desc. Domingo").Width = 100
        DGV_CompleteWeekInfo.Columns("Desc. Domingo").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        DGV_CompleteWeekInfo.Columns("Desc. Domingo").ToolTipText = "Descuento del proporcional de domingo por faltas"
        ...
        'DGV_CompleteWeekInfo.Columns("B. Comida Final").Width = 100
        'DGV_CompleteWeekInfo.Columns("B. Comida Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        'DGV_CompleteWeekInfo.Columns("B. Comida Final").ToolTipText = "Monto de bono de comida descontando días de falta"
        ...
        DGV_CompleteWeekInfo.Columns("Bono BP Final").Width = 100
        DGV_CompleteWeekInfo.Columns("Bono BP Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        DGV_CompleteWeekInfo.Columns("Bono BP Final").ToolTipText = "Monto real del bono de buenas prácticas que se paga esta semana"
        ...
        DGV_CompleteWeekInfo.Columns("Dias Amonest. a Descontar").Width = 90
        DGV_CompleteWeekInfo.Columns("Dias Amonest. a Descontar").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Dias Amonest. a Descontar").ToolTipText = "Días de amonestación que se descontarán realmente esta semana al confirmar la nómina"
        DGV_CompleteWeekInfo.Columns("Dias Amonest. a Descontar").Visible = False

        DGV_CompleteWeekInfo.Columns("Dias Excedente Amonest.").Width = 90
        DGV_CompleteWeekInfo.Columns("Dias Excedente Amonest.").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Dias Excedente Amonest.").ToolTipText = "Días que se acumulan como amonestación por F, FJ, R, PSG"

        DGV_CompleteWeekInfo.Columns("Desc. Amonest. BP").Width = 100
        DGV_CompleteWeekInfo.Columns("Desc. Amonest. BP").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        DGV_CompleteWeekInfo.Columns("Desc. Amonest. BP").ToolTipText = "Descuento al Bono BP por amonestaciones"
        ...
        'DGV_CompleteWeekInfo.Columns("Amonest. por Falta").ToolTipText = "Días de amonestación por faltas"
        ...
        DGV_CompleteWeekInfo.Columns("Monto D. Transporte").Width = 90
        DGV_CompleteWeekInfo.Columns("Monto D. Transporte").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Monto D. Transporte").ToolTipText = "Monto pagado por cada día de transporte"
        ...
        'DGV_CompleteWeekInfo.Columns("Desc. Transporte").Width = 80
        'DGV_CompleteWeekInfo.Columns("Desc. Transporte").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'DGV_CompleteWeekInfo.Columns("Desc. Transporte").ToolTipText = "Monto descontado de transporte por faltas injustificadas"

        'DGV_CompleteWeekInfo.Columns("Transporte Final").Width = 80
        'DGV_CompleteWeekInfo.Columns("Transporte Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'DGV_CompleteWeekInfo.Columns("Transporte Final").ToolTipText = "Monto final de transporte ya con descuento aplicado"
        ...
        DGV_CompleteWeekInfo.Columns("Transporte entre Empleados").Width = 80
        DGV_CompleteWeekInfo.Columns("Transporte entre Empleados").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Transporte entre Empleados").ToolTipText = "Transporte entre Empleados: monto adicional capturado manualmente en Movimientos Generales"

        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 1").Width = 90
        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 1").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 1").ToolTipText = "Bono de transporte entre empleados fijo 1 ($250)"

        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 2").Width = 90
        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 2").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        'DGV_CompleteWeekInfo.Columns("Transporte Fijo 2").ToolTipText = "Bono de transporte entre empleados fijo 2 ($500)"

        'DGV_CompleteWeekInfo.Columns("Desc. Transporte entre Empleados").Width = 90
        'DGV_CompleteWeekInfo.Columns("Desc. Transporte entre Empleados").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'DGV_CompleteWeekInfo.Columns("Desc. Transporte entre Empleados").ToolTipText = "Monto descontado de transporte entre empleados por faltas injustificadas"

        'DGV_CompleteWeekInfo.Columns("Transporte entre Empleados Final").Width = 90
        'DGV_CompleteWeekInfo.Columns("Transporte entre Empleados Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'DGV_CompleteWeekInfo.Columns("Transporte entre Empleados Final").ToolTipText = "Monto final de transporte entre empleados ya con descuento aplicado"
        ...
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Temp").Width = 80
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Temp").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Temp").ToolTipText = "Monto descontado de botonero temporal por faltas injustificadas"

        DGV_CompleteWeekInfo.Columns("Botonero Temp Final").Width = 80
        DGV_CompleteWeekInfo.Columns("Botonero Temp Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Botonero Temp Final").ToolTipText = "Monto neto de botonero temporal ya con descuento aplicado"
        ...
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Fijo").Width = 80
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Fijo").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Desc. Botonero Fijo").ToolTipText = "Monto descontado de botonero fijo por faltas injustificadas"

        DGV_CompleteWeekInfo.Columns("Botonero Fijo Final").Width = 80
        DGV_CompleteWeekInfo.Columns("Botonero Fijo Final").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Botonero Fijo Final").ToolTipText = "Monto final de botonero fijo ya con descuento aplicado"
        ...
        DGV_CompleteWeekInfo.Columns("No. Horas A.").Width = 80
        DGV_CompleteWeekInfo.Columns("No. Horas A.").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("No. Horas A.").ToolTipText = "Cantidad de horas ausentes compensadas vía nómina"

        DGV_CompleteWeekInfo.Columns("Desc. Horas A.").Width = 100
        DGV_CompleteWeekInfo.Columns("Desc. Horas A.").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        DGV_CompleteWeekInfo.Columns("Desc. Horas A.").ToolTipText = "Descuento por horas ausentes compensadas vía nómina"
        ...
        'Columnas en negrita que son parte del cálculo
        Dim columnasCalculo As String() = {
                                "S. Base",
                                "S. Diario",
                                "Monto por Asistencias",
                                "Desc. Domingo",
                                "Monto H. D",
                                "Monto H. T",
                                "Monto Total Comida",
        ...
        For Each colName As String In columnasCalculo
            If DGV_CompleteWeekInfo.Columns.Contains(colName) Then
                DGV_CompleteWeekInfo.Columns(colName).HeaderCell.Style.Font =
            New System.Drawing.Font(DGV_CompleteWeekInfo.Font, FontStyle.Bold)
            End If
        Next
        ...
        For Each row As DataGridViewRow In DGV_CompleteWeekInfo.Rows
            ...
            For i = 8 To DGV_CompleteWeekInfo.Columns.Count - 1
                ...
                Select Case moveID
                    ...
                    '    row.Cells(i).ToolTipText = "Retardo"
                    Case 140
                        row.Cells(i).Value = "A" ' Retardo Justificado — se cuenta como Asistencia completa
                        row.Cells(i).Style.BackColor = System.Drawing.Color.FromArgb(CByte(192), CByte(255), CByte(192))   
                        row.Cells(i).ToolTipText = "Asistencia (retardo justificado)"
                    Case 500
    ...
    Private Sub GetAdditionalValues()
        ...
        For Each EmployeeCum As DataRow In CumulativesEmployees.Rows
            ...
            LB_Progress.Refresh()
            Dim MontoComidaFinal As Decimal = 0.0
            ...
            Dim BotoneroTempAmmount As Decimal = 0.0
            Dim BotoneroFijoAmmount As Decimal = 0.0
            Dim TransportAmmount As Decimal = 0.0
            'Dim TransporteFijo1Ammount As Decimal = 0.0
            'Dim TransporteFijo2Ammount As Decimal = 0.0
            ...
            If CounterR >= 3 Then
                DGV_CompleteWeekInfo.Item("Nombre Completo", CounterLine).ToolTipText = "⚠️ " & CounterR & " retardos esta semana. Se recomienda hablar con el empleado."
                EmpleadosAlerta.Add("ID " & EmployeeID & " - " & DGV_CompleteWeekInfo.Item("Nombre Completo", CounterLine).Value.ToString() & " (" & CounterR & " retardos)")
            End If

            DGV_CompleteWeekInfo.Item("S. Diario", CounterLine).Value = DailySalary.ToString("C2")
            'ExtraS = DailySalary / 48
            ExtraS = BaseSalary / 48

            ' Descuento de domingo por faltas injustificadas Y justificadas
            'Dim DescDomingo As Decimal = (SundaySalary / 7) * (CounterF + CounterFJ)
            Dim DescDomingo As Decimal = (SundaySalary / 7) * (CounterF + CounterFJ + CounterPSG)
            If DescDomingo > 0 Then
                DGV_CompleteWeekInfo.Item("Desc. Domingo", CounterLine).Value = DescDomingo.ToString("C2")
            End If

            'Dim MontoPorAsistencias As Decimal = If(CounterA = 6, BaseSalary, DailySalary * CounterA)
            'Dim MontoPorAsistencias As Decimal = If(CounterA = 6, BaseSalary, (DailySalary * (CounterA + 1)) - DescDomingo)
            'Dim MontoPorAsistencias As Decimal = If(CounterA + CounterR = 6, BaseSalary, (DailySalary * (CounterA + CounterR + 1)) - DescDomingo)
            Dim MontoPorAsistencias As Decimal = If(CounterA + CounterR + CounterPG + CounterV = 6, BaseSalary, (DailySalary * (CounterA + CounterR + CounterPG + CounterV + 1)) - DescDomingo)
            ...
            ' Descuento por horas ausentes compensadas vía nómina
            Dim HorasAusenciaData As DataTable = New CL_RecordByEmployeeHoursAbsence().Get_HoursAbsenceByEmployee(DTP_StartDate.Value, DTP_EndDate.Value, EmployeeID, 516)
            Dim TotalHorasAusencia As Decimal = 0.0
            If HorasAusenciaData.Rows.Count > 0 Then
                TotalHorasAusencia = CDec(HorasAusenciaData.Rows(0)("HORAS_NOMINA"))
            End If

            DGV_CompleteWeekInfo.Item("No. Horas A.", CounterLine).Value = TotalHorasAusencia.ToString("N2")

            Dim DescHorasAusencia As Decimal = (DailySalary / 9) * TotalHorasAusencia
            DGV_CompleteWeekInfo.Item("Desc. Horas A.", CounterLine).Value = DescHorasAusencia.ToString("C2")
            ...
            ' Adeudos - Descuento
            ' respetr abonos manuales
            Dim CLDebt As New CL_EmployeeAmountDebt
            Dim DebtRecords As DataTable = CLDebt.GetDebtsHistory(EmployeeID)
            Dim DiscountAmountValue As Decimal = 0.0
            Dim BalanceAmountValue As Decimal = 0.0

            For Each DebtRow As DataRow In DebtRecords.Rows
                ' Solo considerar adeudos activos
                If DebtRow("Estado").ToString() = "Activo" Then
                    Dim saldoActual As Decimal = CDec(DebtRow("Saldo Pendiente"))
                    Dim descuentoPeriodico As Decimal = CDec(DebtRow("Descuento Periódico"))

                    If saldoActual > 0 Then
                        ' Si el saldo es menor que el descuento periódico, solo descontar lo que queda
                        DiscountAmountValue += Math.Min(saldoActual, descuentoPeriodico)
                        BalanceAmountValue += saldoActual
                    End If
                End If
            Next
            ...
            Dim ProdPlantPercentValue As Integer = 0
            Integer.TryParse(TB_ProdPlant.Text.Trim(), ProdPlantPercentValue)

            Dim ProdPlantConstant As Decimal = 0.0
            If ProdPlantPercentValue >= 80 And ProdPlantPercentValue <= 89 Then
                ProdPlantConstant = 0.2
            ElseIf ProdPlantPercentValue >= 90 And ProdPlantPercentValue <= 100 Then
                ProdPlantConstant = 0.3
            End If
            ...
            For Each Benefit As DataRow In ListOfBenefits.Rows
                ...
                Select Case BenefitID
                    ...
                    '    ' Contar días de F/FJ que NO caen en sábado (el bono de comida no se paga sábado, no se descuenta ese día)
                    '    Dim DiasFaltaNoSabado As Integer = 0
                    '    For d As Integer = 0 To 6
                    '        Dim colDate As Date = DTP_StartDate.Value.AddDays(d)
                    '        Dim cellVal = DGV_CompleteWeekInfo.Rows(CounterLine).Cells(d + 8).Value
                    '        If cellVal IsNot Nothing AndAlso Not IsDBNull(cellVal) Then
                    '            Dim cellText As String = cellVal.ToString()
                    '            If (cellText = "F" OrElse cellText = "FJ") AndAlso colDate.DayOfWeek <> DayOfWeek.Saturday Then
                    '                DiasFaltaNoSabado += 1
                    '            End If
                    '        End If
                    '    Next

                    '    ' Bono de comida final, descontando solo los días de falta que no caigan en sábado
                    '    'Dim MontoComidaFinal As Decimal = Math.Max(0, LunchHourAmmount * (LunchHours - DiasFaltaNoSabado))
                    '    MontoComidaFinal = Math.Max(0, LunchHourAmmount * (LunchHours - DiasFaltaNoSabado))
                    '    DGV_CompleteWeekInfo.Item("B. Comida Final", CounterLine).Value = MontoComidaFinal.ToString("C2")
                    ...
                    Case 40 ' Bono de comida
                        ...
                        MontoComidaFinal = LunchHourAmmount * LunchHours
                    ...
                    '    If TBenefitDetails.Rows.Count > 0 Then
                    '        DGV_CompleteWeekInfo.Item("Monto D. Transporte", CounterLine).Value = BenefitAmmount.ToString("C2")
                    '        DGV_CompleteWeekInfo.Item("D. Transporte", CounterLine).Value = TDaysQuantity

                    '        Dim TransporteBruto As Decimal = BenefitAmmount * TDaysQuantity
                    '        Dim DescTransporte As Decimal = (TransporteBruto / 6) * CounterF
                    '        TransportAmmount = Math.Max(0, TransporteBruto - DescTransporte)

                    '        DGV_CompleteWeekInfo.Item("Transporte", CounterLine).Value = TransporteBruto.ToString("C2")
                    '        DGV_CompleteWeekInfo.Item("Desc. Transporte", CounterLine).Value = DescTransporte.ToString("C2")
                    '        DGV_CompleteWeekInfo.Item("Transporte Final", CounterLine).Value = TransportAmmount.ToString("C2")
                    '    End If
                    ...
                    Case 60 ' Bono de transporte autobus empresarial
                        ...
                        If TBenefitDetails.Rows.Count > 0 Then
                            ...
                            TransportAmmount = BenefitAmmount * TDaysQuantity

                            DGV_CompleteWeekInfo.Item("Transporte", CounterLine).Value = TransportAmmount.ToString("C2")
                        End If

                    Case 70, 260 ' Bono de transporte entre empleados
                        ...
                        If TBenefitDetails.Rows.Count > 0 Then
                            ...
                            'Dim DescTransporteEntreEmp As Decimal = (TEmployeesAmount / 6) * CounterF
                            'TransporteEntreEmpFinal = Math.Max(0, TEmployeesAmount - DescTransporteEntreEmp)

                            DGV_CompleteWeekInfo.Item("Transporte entre Empleados", CounterLine).Value = TEmployeesAmount.ToString("C2")

                        End If

                    'Case 70 ' Bono de transporte entre empleados fijo 1 ($250)
                    '    Dim BenefDetail As New CL_Benefits
                    '    BenefDetail.BENEF_ID = BenefitID
                    '    BenefDetail.EMPL_ID = EmployeeID
                    '    Dim TBenefitDetails As DataTable = BenefDetail.Get_BenefitIDDetailsByEmployee()
                    ...
                    '    If TBenefitDetails.Rows.Count > 0 Then
                    '        TransporteFijo1Ammount = BenefitAmmount
                    '        DGV_CompleteWeekInfo.Item("Transporte Fijo 1", CounterLine).Value = TransporteFijo1Ammount.ToString("C2")
                    '    End If

                    'Case 260 ' Bono de transporte entre empleados fijo 2 ($500)
                    '    Dim BenefDetail As New CL_Benefits
                    '    BenefDetail.BENEF_ID = BenefitID
                    '    BenefDetail.EMPL_ID = EmployeeID
                    '    Dim TBenefitDetails As DataTable = BenefDetail.Get_BenefitIDDetailsByEmployee()
                    ...
                    '    If TBenefitDetails.Rows.Count > 0 Then
                    '        TransporteFijo2Ammount = BenefitAmmount
                    '        DGV_CompleteWeekInfo.Item("Transporte Fijo 2", CounterLine).Value = TransporteFijo2Ammount.ToString("C2")
                    '    End If

                    Case 90, 100, 110, 120, 130, 140, 150, 160, 170, 240 ' Bono de buenas practicas o actitud
                    ...
                    Case 200 'Bono de productividad de planta 80%
                        ...
                        If TBenefitDetails.Rows.Count > 0 Then
                            ...
                            '' OPCIÓN B  el % de la semana aplica a cualquiera con beneficio de planta 
                            'ProdPlantTotalAmmount = (BaseSalary * ProdPlantConstant / 6) * (6 - CounterF - CounterFJ)

                            ' OPCIÓN A respeta el nivel asignado al empleado
                            If ProdPlantPercentValue >= 80 And ProdPlantPercentValue <= 89 Then
                                ProdPlantTotalAmmount = (BaseSalary * 0.2 / 6) * (6 - CounterF - CounterFJ)
                            Else
                                ProdPlantTotalAmmount = 0
                            End If
                        End If
                    ...
                    Case 210 'Bono de productividad de planta 90%
                        ...
                        If TBenefitDetails.Rows.Count > 0 Then
                            ...
                            ' OPCIÓN A 
                            If ProdPlantPercentValue >= 90 And ProdPlantPercentValue <= 100 Then
                                ProdPlantTotalAmmount = (BaseSalary * 0.3 / 6) * (6 - CounterF - CounterFJ)
                            Else
                                ProdPlantTotalAmmount = 0   ' esta semana está en modo 20%, este empleado no aplica
                            End If
                        End If
                    ...
                    Case 220 'Bono de Botonero temporal
                        ...
                        Dim TBenefitDetails As DataTable = BenefDetail.Get_BenefitIDDetailsByEmployee()
                        If TBenefitDetails.Rows.Count > 0 Then
                            Dim DescBotoneroTemp As Decimal = (BenefitAmmount / 6) * CounterF
                            BotoneroTempAmmount = (BenefitAmmount / 6) * CounterA
                            DGV_CompleteWeekInfo.Item("Monto B. Botonero Temp", CounterLine).Value = BenefitAmmount.ToString("C2")
                            DGV_CompleteWeekInfo.Item("Desc. Botonero Temp", CounterLine).Value = DescBotoneroTemp.ToString("C2")
                            DGV_CompleteWeekInfo.Item("Botonero Temp Final", CounterLine).Value = BotoneroTempAmmount.ToString("C2")
                    ...
                    Case 230 'Bono de Botonero definitivo
                        ...
                        If TBenefitDetails.Rows.Count > 0 Then
                            Dim TotalFaltasBotonero As Integer = CounterF + CounterFJ
                            Dim DescBotoneroFijo As Decimal = (BenefitAmmount / 6) * TotalFaltasBotonero
            ...
            ' Si ningún ID de BP coincidió con este empleado, mostrar $0.00 explícitamente (una sola vez, no por cada iteración)
            If AttitudeGoodPract = 0 Then
                DGV_CompleteWeekInfo.Item("Bono BP", CounterLine).Value = (0).ToString("C2")
            End If
            ...
            If ProdPlantPercentValue >= 80 And ProdPlantPercentValue <= 100 And (PropPlantAmmount_1 <> 0 Or PropPlantAmmount_2 <> 0) Then
                ...
                If ProdPlantPercentValue >= 80 And ProdPlantPercentValue <= 89 Then
                    DGV_CompleteWeekInfo.Item("Bono P. P.", CounterLine).ToolTipText = "Planta al " & ProdPlantPercentValue & "% → aplica bono del 20% del salario base"
                ElseIf ProdPlantPercentValue >= 90 And ProdPlantPercentValue <= 100 Then
                    DGV_CompleteWeekInfo.Item("Bono P. P.", CounterLine).ToolTipText = "Planta al " & ProdPlantPercentValue & "% → aplica bono del 30% del salario base"
                End If
            End If

            ''Hice cambio
            'AMONESTACIONES - Ahora afectan solo el Bono BP, NO el de productividad
            ...
            ' Bono Productividad — siempre completo, solo afectan las faltas injustificadas
            BonoProdNeto = ProductivityAmmount

            'Bono Prod Final — descuenta días de falta injustificada Y de falta justificada 
            'Dim BonoProdFinal As Decimal = ProductivityAmmount - ((ProductivityAmmount / 6) * CounterF)
            Dim BonoProdFinal As Decimal = ProductivityAmmount - ((ProductivityAmmount / 6) * (CounterF + CounterFJ))
            ...
            ' Bono BP para FALTAS JUSTIFICADAS y quitando amonestaciones si es que tiene el empleado
            Dim DiasDisponiblesAmonestacionFJ As Decimal = Math.Max(0, 6 - CounterFJ)
            Dim DiasAmonestacionADescontarFJ As Decimal = Math.Min(DiasPendientes, DiasDisponiblesAmonestacionFJ)
            Dim BannDiscountBP_FJ As Decimal = (AttitudeGoodPract / 6) * DiasAmonestacionADescontarFJ
            Dim TotalDiasDescuentoBP_FJ As Decimal = Math.Min(CounterFJ + DiasAmonestacionADescontarFJ, 6)
            Dim BonoBPFinalFJ As Decimal = AttitudeGoodPract - ((AttitudeGoodPract / 6) * TotalDiasDescuentoBP_FJ)
            If BonoBPFinalFJ < 0 Then BonoBPFinalFJ = 0

            ' Bono BP Final — pierde todo si hay faltas, y descuenta amonestaciones (÷6)
            Dim BannDiscountBP As Decimal = 0.0D
            If AttitudeGoodPract > 0 AndAlso DiasADescontar > 0 Then
                BannDiscountBP = (AttitudeGoodPract / 6) * DiasADescontar
            End If
            'Dim BonoBPFinal As Decimal = If(CounterF > 0, 0, AttitudeGoodPract - BannDiscountBP)
            Dim BonoBPFinal As Decimal
            If CounterF > 0 Then
                BonoBPFinal = 0
            ElseIf CounterFJ > 0 Then
                BonoBPFinal = BonoBPFinalFJ
            ElseIf CounterR >= 3 Then
                BonoBPFinal = 0                                                          ' 3+ retardos: pierde todo el BP
            ElseIf CounterR = 2 Then
                BonoBPFinal = Math.Max(0, AttitudeGoodPract - (AttitudeGoodPract / 6))   ' 2 retardos -1 día de BP
            Else
                BonoBPFinal = Math.Max(0, AttitudeGoodPract - BannDiscountBP)            ' 0-1 retardos completo (menos amonestaciones si aplica)
            End If
            If BonoBPFinal < 0 Then BonoBPFinal = 0
            DGV_CompleteWeekInfo.Item("Bono BP Final", CounterLine).Value = BonoBPFinal.ToString("C2")

            ' Descuento de amonestaciones en BP
            'If BannDiscountBP > 0 Then
            '    DGV_CompleteWeekInfo.Item("Desc. Amonest. BP", CounterLine).Value = BannDiscountBP.ToString("C2")
            'End If



            'Bono Productividad para FALTAS JUSTIFICADAS 
            Dim BonoProdFinalFJ As Decimal = ProductivityAmmount - ((ProductivityAmmount / 6) * CounterFJ)
            If BonoProdFinalFJ < 0 Then BonoProdFinalFJ = 0

            ' Solo sobreescribir la columna si el empleado realmente tiene faltas justificadas esta semana
            'If CounterFJ > 0 Then
            If CounterFJ > 0 AndAlso CounterF = 0 Then
                DGV_CompleteWeekInfo.Item("Bono BP Final", CounterLine).Value = BonoBPFinalFJ.ToString("C2")
            End If


            ' Bono Productividad y BP para PERMISO CON GOCE, PERMISO SIN GOCE Y VACACIONES 
            Dim DiasReduceBonosPV As Integer = CounterPSG + CounterPG + CounterV

            Dim BonoProdFinalPV As Decimal = ProductivityAmmount - ((ProductivityAmmount / 6) * DiasReduceBonosPV)
            If BonoProdFinalPV < 0 Then BonoProdFinalPV = 0

            Dim DiasDisponiblesAmonestacionPV As Decimal = Math.Max(0, 6 - DiasReduceBonosPV)
            Dim DiasAmonestacionADescontarPV As Decimal = Math.Min(DiasPendientes, DiasDisponiblesAmonestacionPV)
            Dim TotalDiasDescuentoBP_PV As Decimal = Math.Min(DiasReduceBonosPV + DiasAmonestacionADescontarPV, 6)
            Dim BonoBPFinalPV As Decimal = AttitudeGoodPract - ((AttitudeGoodPract / 6) * TotalDiasDescuentoBP_PV)
            If BonoBPFinalPV < 0 Then BonoBPFinalPV = 0

            ' Calculos para "Casos Combinados/mixtos": combina F, FJ, PSG, PG, V y retardos 
            Dim DiasReduceBonosGeneral As Integer = CounterFJ + CounterPSG + CounterPG + CounterV
            Dim DiasDisponiblesAmonestacionGeneral As Decimal = Math.Max(0, 6 - DiasReduceBonosGeneral)
            Dim DiasAmonestacionADescontarGeneral As Decimal = Math.Min(DiasPendientes, DiasDisponiblesAmonestacionGeneral)
            Dim TotalDiasDescuentoBP_General As Decimal = Math.Min(DiasReduceBonosGeneral + DiasAmonestacionADescontarGeneral, 6)

            ' F + retardos + FJ/PSG/PG/V que no cupieron en el BP
            Dim DiasRetardosPeso As Integer = If(CounterR >= 3, 6, If(CounterR = 2, 1, 0))
            Dim DiasFPeso As Integer = If(CounterF > 0, 6, 0)
            Dim DiasOcupadosSemana As Integer = DiasFPeso + DiasRetardosPeso + DiasReduceBonosGeneral
            Dim DiasExcedenteSemana As Decimal = Math.Max(0, DiasOcupadosSemana - 6)
            DGV_CompleteWeekInfo.Item("Dias Excedente Amonest.", CounterLine).Value = DiasExcedenteSemana

            Dim BonoBPFinalGeneral As Decimal
            If CounterF > 0 Then
                BonoBPFinalGeneral = 0            ' cualquier falta injustificada: BP en $0
            ElseIf CounterR >= 3 Then
                BonoBPFinalGeneral = 0             ' 3+ retardos: BP en $0
            ElseIf CounterR = 2 Then
                BonoBPFinalGeneral = Math.Max(0, AttitudeGoodPract - ((AttitudeGoodPract / 6) * (1 + DiasReduceBonosGeneral)))  ' 2 retardos (-1 día) + FJ/PSG/PG/V
            Else
                BonoBPFinalGeneral = AttitudeGoodPract - ((AttitudeGoodPract / 6) * TotalDiasDescuentoBP_General)
            End If
            If BonoBPFinalGeneral < 0 Then BonoBPFinalGeneral = 0
            ...
            'días de amonestación que realmente se descontarán esta semana 
            Dim DiasAmonestacionSemana As Decimal = 0.0D
            If CounterF > 0 Then
                DiasAmonestacionSemana = 0
            ElseIf CounterFJ > 0 Then
                DiasAmonestacionSemana = DiasAmonestacionADescontarFJ
            Else
                DiasAmonestacionSemana = DiasADescontar
            End If
            DGV_CompleteWeekInfo.Item("Dias Amonest. a Descontar", CounterLine).Value = DiasAmonestacionSemana

            Dim DescAmonestacionBPSemana As Decimal = (AttitudeGoodPract / 6) * DiasAmonestacionSemana
            DGV_CompleteWeekInfo.Item("Desc. Amonest. BP", CounterLine).Value = DescAmonestacionBPSemana.ToString("C2")

            Dim BotoneroTotalAmmount As Decimal = BotoneroTempAmmount + BotoneroFijoAmmount
            ...
            NewSalary = MainSalaryCalculation(CounterA, CounterF, CounterFJ, CounterR, CounterPG, CounterPSG, CounterV, BaseSalary,
                                              ...
                                              SavingAmmount, PaymentAmmount, LunchHours, BotoneroTotalAmmount, TransportAmmount,
    ...
    Private Function CountLetterByEmployee(ByVal EmployeeNo As Integer, ByVal Letter As String) As Integer
        ...
        If DGV_CompleteWeekInfo.Rows.Count = 0 Then Return 0
    ...
    Private Function MainSalaryCalculation(ByVal counterA As Integer, ByVal counterF As Integer, ByVal counterFJ As Integer, ByVal counterR As Integer,
                                                ...
                                                ByVal SavingAmmount As Decimal, ByVal PaymentAmmount As Decimal, ByVal LunchHours As Decimal,
                                                ByVal BotoneroTotalAmmount As Decimal, ByVal TransportAmmount As Decimal, ByVal TransportBetweenAmmount As Decimal,
                                                ByVal BonoProdNeto As Decimal, ByVal ProdPlantTotalAmmount As Decimal, ByVal DiscountAmountValue As Decimal,
                                                ByVal MontoExtraDoble As Decimal, ByVal MontoExtraTriple As Decimal, ByVal infonavitAmountValue As Decimal,
                                                ByVal BaseSalaryFor6Days As Decimal, ByVal BonoProdFinal As Decimal, ByVal BonoBPFinal As Decimal,
                                                ByVal BonoBPFinalFJ As Decimal, ByVal BonoProdFinalFJ As Decimal, ByVal MontoComidaFinal As Decimal,
                                                ByVal BonoBPFinalPV As Decimal, ByVal BonoProdFinalPV As Decimal, ByVal BonoBPFinalGeneral As Decimal,
                                                ByVal BonoProdFinalGeneral As Decimal, ByVal DescHorasAusencia As Decimal) As Decimal
        Dim NewSalary As Decimal = 0.0
        Dim CasoCubierto As Boolean = False
        ...
        Select Case counterA ' Number of assistance
            Case 6
                ...
                'NewSalary = BaseSalaryFor6Days + AttitudeGoodPract + SundaySalary
                ...
                NewSalary = NewSalary + BonoProdNeto                    ' Bono de productividad completo
                NewSalary = NewSalary + BonoBPFinal                     ' BP - amonestaciones
                NewSalary = NewSalary + ProdPlantTotalAmmount           ' agregamos el monto por productividad de la planta
                NewSalary = NewSalary + BotoneroTotalAmmount             ' agregamos el biono de botonero
                NewSalary = NewSalary + MontoExtraDoble                 ' Horas extra dobñe
                NewSalary = NewSalary + MontoExtraTriple                ' Horas extra Triple 
                NewSalary = NewSalary + TransportAmmount                ' Agregamos bono de transporte
                NewSalary = NewSalary + TransportBetweenAmmount         ' Transporte
                ...
                NewSalary = NewSalary - infonavitAmountValue            ' Descontamos el monto de infonavit
                NewSalary = NewSalary - DescHorasAusencia                ' Descuento por horas ausentes vía nómina
            Case Else
                ...
                If counterA = 5 And counterF = 1 Then
                    ...
                    'NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * (LunchHours - 1))    ' Horas de comida 
                    ...
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                    NewSalary = NewSalary + MontoExtraTriple                                    ' Horas extra Triple
                    NewSalary = NewSalary + BonoProdFinal                                       ' Bono de BProd ya con los días descontados de sus faltas
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' agregamos el monto por productividad de la planta
                    CasoCubierto = True
                End If
                ...
                If counterA = 4 And counterF = 2 Then
                    ...
                    'NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * (LunchHours - 2))    ' Horas de comida
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + 0                                                   ' se le descuenta el bono de todas la semana del bono de buens practicas (+ 0)
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                ...
                If counterA = 3 And counterF = 3 Then
                    ...
                    'NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * (LunchHours - 3))    ' Horas de comida
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + 0                                                   ' se le descuenta el bono de todas la semana del bono de buens practicas (+ 0)
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    ...
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                    NewSalary = NewSalary + MontoExtraTriple                                    ' Horas extra triple
                    NewSalary = NewSalary + BonoProdFinal                                       ' ya incluye descuento de 3 faltas                                    
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' agregamos el monto por productividad de la planta
                    CasoCubierto = True
                End If
                ...
                If counterA = 2 And counterF = 4 Then
                    ...
                    'NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * (LunchHours - 4))    ' Horas de comida
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + 0                                                   ' se le descuenta el bono de todas la semana del bono de buens practicas (+ 0)
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    ...
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                    NewSalary = NewSalary + MontoExtraTriple                                    ' Horas extra Triple
                    NewSalary = NewSalary + BonoProdFinal                                       ' ya incluye descuento de 4 faltas                                     
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' agregamos el monto por productividad de la planta
                    CasoCubierto = True
                End If
                ...
                If counterA = 1 And counterF = 5 Then
                    ...
                    'NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * (LunchHours - 5))    ' Horas de comida
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + 0                                                   ' se le descuenta el bono de todas la semana del bono de buens practicas (+ 0)
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    ...
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                    NewSalary = NewSalary + MontoExtraTriple                                    ' Horas extra triple
                    NewSalary = NewSalary + BonoProdFinal                                       ' ya incluye descuento de 5 faltas                                       
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' agregamos el monto por productividad de la planta
                    CasoCubierto = True
                End If
                ...
                'If counterA = 0 And counterF = 6 Then
                '    NewSalary = DailySalary * 1
                '    NewSalary = NewSalary - ((SundaySalary / 7) * 6)
                ...
                If counterA = 5 And counterFJ = 1 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta un día del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta un día del BP 
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta un día del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterA = 4 And counterFJ = 2 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta 2 días del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta 2 días del BP
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta 2 días del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterA = 3 And counterFJ = 3 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta 3 días del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta 3 días del BP
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta 3 días del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterA = 2 And counterFJ = 4 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta 4 días del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta 4 días del BP
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta 4 días del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterA = 1 And counterFJ = 5 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta 5 días del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta 5 días del BP
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta 5 días del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterA = 0 And counterFJ = 6 Then
                    ...
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalFJ                                     ' Se le descuenta 6 días del B.Prod
                    NewSalary = NewSalary + BonoBPFinalFJ                                       ' Se le descuenta 6 días del BP
                    NewSalary = NewSalary + ProdPlantTotalAmmount                               ' Se le descuenta 6 días del Bono de Prod. Planta
                    NewSalary = NewSalary - SavingAmmount                                       ' Menos la cantidad a ahorrar
                    NewSalary = NewSalary - PaymentAmmount                                      ' Menos el pago por créditos
                    NewSalary = NewSalary - DiscountAmountValue                                 ' Descuento por adeudo
                    NewSalary = NewSalary + MontoExtraDoble                                     ' Horas extra dobñe
                ...
                If counterR = 1 And counterF = 0 And counterFJ = 0 Then
                    ...
                    NewSalary = NewSalary + AttitudeGoodPract                    ' BP completo, sin descuento
                    NewSalary = NewSalary + ProdPlantTotalAmmount
                ...
                If counterR = 2 And counterF = 0 And counterFJ = 0 Then
                    ...
                    NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * LunchHours)
                    NewSalary = NewSalary + BonoProdNeto
                    NewSalary = NewSalary + BonoBPFinalR2
                    NewSalary = NewSalary + ProdPlantTotalAmmount
                    NewSalary = NewSalary + MontoExtraDoble
                    NewSalary = NewSalary + MontoExtraTriple
                    NewSalary = NewSalary - SavingAmmount
                ...
                If counterR = 3 And counterF = 0 And counterFJ = 0 Then
                    ...
                    NewSalary = NewSalary + Math.Max(0, LunchHourAmmount * LunchHours)
                    NewSalary = NewSalary + BonoProdNeto
                    NewSalary = NewSalary + 0                                    ' BP completo perdido
                    NewSalary = NewSalary + ProdPlantTotalAmmount
                    NewSalary = NewSalary + MontoExtraDoble
                    NewSalary = NewSalary + MontoExtraTriple
                    NewSalary = NewSalary - SavingAmmount
                ...
                If counterR = 4 And counterF = 0 And counterFJ = 0 Then
                    ...
                    NewSalary = DailySalary * (DiasTrabajados4R + 1)
                    NewSalary = NewSalary - ((SundaySalary / 7) * DiasFalta4R)                                  ' no se paga domingo de ese día
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + Math.Max(0, BonoProdNeto - (BonoProdNeto / 6))                      ' se pierde 1 día de productividad
                    NewSalary = NewSalary + Math.Max(0, AttitudeGoodPract - (AttitudeGoodPract / 6))            ' se pierde 1 día de BP                                                                ' BP completo perdido (cuenta como falta)
                    NewSalary = NewSalary + Math.Max(0, ProdPlantTotalAmmount - (ProdPlantTotalAmmount / 6))    ' se pierde 1 día de planta
                    NewSalary = NewSalary - (BotoneroTotalAmmount / 6)                                          ' se pierde 1 día de botonero 
                    NewSalary = NewSalary + MontoExtraDoble
                    NewSalary = NewSalary + MontoExtraTriple
                    NewSalary = NewSalary - SavingAmmount
                    NewSalary = NewSalary - PaymentAmmount
                ...
                '' ------------------------ CASOS COMBINADOS (cualquier mezcla no cubierta por una rama exacta arriba)
                ''If NewSalary = 0 And (counterA + counterR + counterF + counterFJ) > 0 Then
                'If Not CasoCubierto And (counterA + counterR + counterF + counterFJ) > 0 Then
                '    Dim DiasTrabajados As Integer = counterA + counterR      ' un retardo sigue siendo día trabajado
                '    Dim DiasFalta As Integer = counterF + counterFJ           ' cualquier tipo de falta reduce el pago

                '    NewSalary = DailySalary * (DiasTrabajados + 1)
                '    NewSalary = NewSalary - ((SundaySalary / 7) * DiasFalta)
                '    NewSalary = NewSalary + MontoComidaFinal
                '    NewSalary = NewSalary + BonoProdFinal
                '    NewSalary = NewSalary + BonoBPFinal
                '    NewSalary = NewSalary - SavingAmmount
                '    NewSalary = NewSalary - PaymentAmmount
                '    NewSalary = NewSalary - DiscountAmountValue
                '    NewSalary = NewSalary + MontoExtraDoble
                ...
                '----------------------------------- PERMISO CON GOCE, PERMISO SIN GOCE Y VACACIONES

                If (counterPG > 0 Or counterPSG > 0 Or counterV > 0) And counterF = 0 And counterFJ = 0 And counterR = 0 Then

                    Dim DiasTrabajadosPV As Integer = counterA + counterPG + counterV     ' PG y V se pagan como día trabajado
                    Dim DiasFaltaPV As Integer = counterPSG                               ' solo PSG resta salario y domingo (igual que FJ)
                    Dim DiasReduceBonosPV As Integer = counterPSG + counterPG + counterV  ' los 3 reducen los demás bonos por igual

                    NewSalary = DailySalary * (DiasTrabajadosPV + 1)
                    NewSalary = NewSalary - ((SundaySalary / 7) * DiasFaltaPV)
                    NewSalary = NewSalary + MontoComidaFinal
                    NewSalary = NewSalary + BonoProdFinalPV
                    NewSalary = NewSalary + BonoBPFinalPV
                    NewSalary = NewSalary + Math.Max(0, ProdPlantTotalAmmount - ((ProdPlantTotalAmmount / 6) * DiasReduceBonosPV))
                    NewSalary = NewSalary - ((BotoneroTotalAmmount / 6) * DiasReduceBonosPV)
                    NewSalary = NewSalary - ((TransportAmmount / 6) * DiasReduceBonosPV)
                    NewSalary = NewSalary + MontoExtraDoble
                    NewSalary = NewSalary + MontoExtraTriple
                    NewSalary = NewSalary - SavingAmmount
                    NewSalary = NewSalary - PaymentAmmount
                ...
                ' Bonos adicionales
                NewSalary = NewSalary + BotoneroTotalAmmount
    ...
    Private Sub BT_FinalConfirmation_Click(sender As Object, e As EventArgs) Handles BT_FinalConfirmation.Click
        ...
        '       Not IsDBNull(Item.Cells(30).Value
        '       ) AndAlso
        ...
        Dim BatchIDActual As String = objP.GetBatchID(DTP_StartDate.Value.Date)
        Dim ultimaAprobacion As DataTable = objP.GetLatestApprovalByWeek(DTP_StartDate.Value.Date, DTP_EndDate.Value.Date)
        If ultimaAprobacion.Rows.Count > 0 Then
            Dim ultimoEstatus As String = ultimaAprobacion.Rows(0)("Status").ToString()
            If ultimoEstatus = "Rechazado" Then
                Dim ultimaVersion As Integer = CInt(ultimaAprobacion.Rows(0)("Version"))
                BatchIDActual &= "-" & ultimaVersion.ToString()
            End If
        End If
        ...
        Try
            ...
            For Each row As DataGridViewRow In DGV_CompleteWeekInfo.Rows
                ...
                objP.AttitudeBonusFinal = ToDec(row.Cells("Bono BP Final").Value)
                objP.ProductivityBonusFinal = ToDec(row.Cells("Bono Prod. Final").Value)
                objP.PlantBonusAmount = ToDec(row.Cells("Monto Bono P. P.").Value)
                objP.TransportBetweenEmployeesBonus = ToDec(row.Cells("Transporte entre Empleados").Value)
                objP.BotoneroTempFinal = ToDec(row.Cells("Botonero Temp Final").Value)
                objP.BotoneroFijoFinal = ToDec(row.Cells("Botonero Fijo Final").Value)
                objP.LoanAmount = ToDec(row.Cells("Prestado").Value)
                objP.LoanPaid = ToDec(row.Cells("Pagado").Value)
                objP.LoanBalance = ToDec(row.Cells("Saldo a pagar").Value)
                objP.HasInfonavit = CBool(row.Cells("Infonavit").Value)
                objP.InfonavitAmount = ToDec(row.Cells("Monto infonavit").Value)
                objP.AbsenceHours = ToDec(row.Cells("No. Horas A.").Value)
                objP.AbsenceHoursDiscount = ToDec(row.Cells("Desc. Horas A.").Value)
                objP.DebtAmount = ToDec(row.Cells("Monto adeudo").Value)
                objP.DebtDiscount = ToDec(row.Cells("Desc. por adeudo").Value)
                objP.DebtBalance = ToDec(row.Cells("Saldo adeudo").Value)
                objP.TransferAmount = ToDec(row.Cells("Monto a transferir").Value)
                objP.CashAmount = ToDec(row.Cells("Monto en efectivo").Value)

                objP.TotalNeto = ToDec(row.Cells("Calculado").Value)
                objP.CreatedBy = AppUser
                objP.BatchID = BatchIDActual

                Dim currentID = objP.InsertPayrollWeek()

                If currentID IsNot Nothing AndAlso IsNumeric(currentID) Then

                    MontoTotalBatch += objP.TotalNeto
                    ...
                    Dim diasCellVal As Object = row.Cells("Dias Amonest. a Descontar").Value
                    Dim diasDescontados As Decimal = 0.0D
                    If diasCellVal IsNot Nothing AndAlso Not IsDBNull(diasCellVal) Then
                        Decimal.TryParse(diasCellVal.ToString(), diasDescontados)
                    End If
                    If diasDescontados > 0 Then
                    ...
                    ' Generar amonestación automática por el excedente de la semana (F, retardos, FJ, PSG/PG/V combinados)
                    Dim excedenteCellVal As Object = row.Cells("Dias Excedente Amonest.").Value
                    Dim diasExcedente As Decimal = 0.0D
                    If excedenteCellVal IsNot Nothing AndAlso Not IsDBNull(excedenteCellVal) Then
                        Decimal.TryParse(excedenteCellVal.ToString(), diasExcedente)
                    End If
                    If diasExcedente > 0 Then
                        Dim CLBannInsert As New CL_EmployeeBanns
                        CLBannInsert.REMPL_ID = CInt(row.Cells("No.").Value)
                        CLBannInsert.DREMPL_DATE = DTP_StartDate.Value.Date
                        CLBannInsert.DREMPL_DQUANTITY = diasExcedente
                        CLBannInsert.DREMPL_BNAME = "Excedente de faltas/retardos"
                        CLBannInsert.DREMPL_DESCR = $"Amonestación automática por {diasExcedente} día(s) que no cupieron en el Bono BP de la semana {DTP_StartDate.Value:dd/MM/yyyy} - {DTP_EndDate.Value:dd/MM/yyyy}."
                        CLBannInsert.DREMPL_CREBY = AppUser
                        CLBannInsert.DREMPL_STATUS = True
            ...
            If savedCount > 0 Then
                objP.InsertPayrollApproval(BatchIDActual, DTP_StartDate.Value.Date, DTP_EndDate.Value.Date,
                                MontoTotalBatch, AppUser)
            End If
    ...
    Private Sub BT_ExportExcel_Click(sender As Object, e As EventArgs) Handles BT_ExportExcel.Click
        Try
            Using workbook As New XLWorkbook()
                ...
                Dim CLBatch As New CL_Payroll
                Dim ultimoBatch As String = CLBatch.GetLatestBatchID(DTP_StartDate.Value.Date, DTP_EndDate.Value.Date)

                Dim sufijoNombre As String = ""
                If Not String.IsNullOrEmpty(ultimoBatch) Then
                    sufijoNombre = "_" & ultimoBatch
                    If ultimoBatch.Contains("-") Then
                        sufijoNombre &= "_CORREGIDA"
                    End If
                End If

                Dim sfd As New SaveFileDialog()
                sfd.Filter = "Excel Files|*.xlsx"
                sfd.FileName = "Nomina_Semana_" & DateTime.Now.ToString("ddMMyyyy") & sufijoNombre
